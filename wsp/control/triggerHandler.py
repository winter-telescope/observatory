...
class TriggerHandler(QtCore.QObject):
    ...
    def __init__(self, config, sunsim = False, logger = None, verbose = False, 
                 timeformat = '%H:%M:%S.%f'):
    ...
    def getTrigCurVals(self, triggercond, sun_alt, timestamp, nextmorning = False):
        ...
        if trigtype == 'sun':
            ...
            if self.verbose:
                self.log(f'trig_datetime_today = {trig_datetime_today}, timestamp = {trig_datetime_today.timestamp()}')
                self.log(f'now_datetime        = {now_datetime}, timestamp = {now_datetime.timestamp()}')
                self.log('')
    ...
    def triggerReady(self, trigname, sun_alt, timestamp, sun_rising)-> bool:
        ...
        for i in range(num_trigs):
            ...
            trig_i = triglist[i]
            trigval, curval = self.getTrigCurVals(triggercond = trig_i, sun_alt = sun_alt, timestamp = timestamp, nextmorning = trig.nextmorning)
        ...
        if all_conditions_met & trig_sun_ok & nextmorning_ok:
            ...
            for i in range(num_trigs):
                ...
                trig_i = triglist[i]
                trig_type = trig_i.trigtype
                trigval, curval = self.getTrigCurVals(triggercond = trig_i, sun_alt = sun_alt, 
                                                      timestamp = timestamp)
                trig_condition = f'{curval} {trig_i.cond} {trigval}'
                trig_condition_met = eval(trig_condition)
                if trig_type == 'sun':
