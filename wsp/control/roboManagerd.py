...
class ReconnectHandler(object):
    ...
    def __init__(self, connection_timeout=0):
        ...
        self.reconnect_timeouts = (
            np.array([0, 0.5, 1, 5]) + self.connection_timeout
        )  # all the allowed timeouts between reconnection attempts CAN'T BE LESS THAN CONN TIMEOUT
    ...
    def get_time_since_last_connection(self):
        ...
        self.reconnect_remaining_time = (
            self.reconnect_timeout - self.time_since_last_connection
        )
...
class StatusMonitor(QtCore.QObject):
    ...
    def __init__(self, proxyname, logger=None, connection_timeout=0.5, verbose=False):
        ...
        self.connection_timeout = (
            connection_timeout  # time to allow each connection attempt to take
        )
    ...
    def create_socket(self):
        ...
        try:
            ...
            self.connected = False
            self.logger.error(
                f"roboManager: connection with remote object failed: {e}", exc_info=True
            )
            pass
    ...
    def connect_socket(self):
        if self.verbose:
            self.log(
                f"(Thread {threading.get_ident()}) StatusMonitor: Attempting to connect socket"
            )
        ...
        try:
            ...
            self.log(
                f"(Thread {threading.get_ident()}) StatusMonitor: trying to connection to ({self.addr} | {self.port})"
            )
            ...
            if self.verbose:
                self.log(
                    f"(Thread {threading.get_ident()}) StatusMonitor: connection unsuccessful: {e}, waiting {self.reconnector.reconnect_timeout} until next reconnection"
                )
    ...
    def pollStatus(self):
        ...
        self.state.update(
            {"reconnect_remaining_time": self.reconnector.reconnect_remaining_time}
        )
...
class CommandHandler(QtCore.QObject):
    ...
    def connect_socket(self):
        # if self.verbose:
        self.log(
            f"CommandHandler: (Thread {threading.get_ident()}) Attempting to connect socket"
        )
        ...
        try:
            ...
            # if self.verbose:
            self.log(
                f"CommandHandler: (Thread {threading.get_ident()}) Connection attempt successful!"
            )
            ...
            # if self.verbose:
            self.log(
                f"CommandHandler: (Thread {threading.get_ident()}) CommandHandler: connection unsuccessful."
            )
    ...
    def sendCommand(self, cmdRequest):
        ...
        if self.connected:
            ...
            try:
                reply = utils.query_socket(self.sock, cmd, end_char="\n")
                ...
                self.alertHandler.slack_log(
                    f":futurama-bender-robot: roboManager: sent command *{cmd}*"
                )
                ...
                # print(f'Query attempt failed.')
                self.log(
                    f"CommandHandler: Tried to send command {cmd} to dome, but rasied exception: {e}"
                )
                self.connected = False
        else:
            self.log(
                f"CommandHandler: Received command: {cmd}, but WSP was disconnected. Reply = {self.disconnectedReply}"
            )
...
class RoboManager(QtCore.QObject):
    ...
    def setupTrigs(self):
        ...
        for trig in self.config["robotic_manager_triggers"]["triggers"]:
            ...
            trigsundir = self.config["robotic_manager_triggers"]["triggers"][trig][
                "sundir"
            ]
            trignextmorning = self.config["robotic_manager_triggers"]["triggers"][
                trig
            ].get("nextmorning", False)
            trigcmd = self.config["robotic_manager_triggers"]["triggers"][trig]["cmd"]
            repeat_on_restart = self.config["robotic_manager_triggers"]["triggers"][
                trig
            ]["repeat_on_restart"]

            for cond in self.config["robotic_manager_triggers"]["triggers"][trig][
                "conds"
            ]:
                print(f"handling condition: {cond}")
                trigtype = self.config["robotic_manager_triggers"]["triggers"][trig][
                    "conds"
                ][cond]["type"]
                trigcond = self.config["robotic_manager_triggers"]["triggers"][trig][
                    "conds"
                ][cond]["cond"]
                trigval = self.config["robotic_manager_triggers"]["triggers"][trig][
                    "conds"
                ][cond]["val"]
    ...
    def setupTrigLog(self):
        ...
        self.triglog_linkpath = os.path.join(
            self.triglog_linkdir, self.triglog_linkname
        )
    ...
    def getTrigCurVals(self, triggercond, nextmorning=False):
        ...
        if trigtype == "sun":
            ...
            trigval = triggercond.val
            trig_datetime = datetime.strptime(
                trigval, self.config["robotic_manager_triggers"]["timeformat"]
            )
    ...
    def handleTrigger(self, trigname):
        ...
        for i in range(num_trigs):
            ...
            trigval, curval = self.getTrigCurVals(
                triggercond=trig_i, nextmorning=trig.nextmorning
            )
        ...
        if all_conditions_met & trig_sun_ok & nextmorning_ok:
            ...
            self.do(
                ...
                time_string=datetime.fromtimestamp(self.state["timestamp"]).isoformat(
                    sep=" "
                ),
            )
...
class PyroGUI(QtCore.QObject):
    ...
    def __init__(self, config, logger=None, verbose=False, parent=None, sunsim=False):
        ...
        self.pyro_thread = daemon_utils.PyroDaemon(
            obj=self.roboManager, name="roboManager"
        )
