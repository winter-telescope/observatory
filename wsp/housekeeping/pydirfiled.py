...
class DirfileWriter(QtCore.QObject):
    ...
    def init_remote_object(self):
        ...
        try:
            if self.verbose:
                print(
                    f'trying to reconnect to remote object "state" at ns_host = {self.ns_host}'
                )
    ...
    def add_to_frame(self, state):
        ...
        for field in self.config["fields"]:
            ...
            try:
                ...
                if self.verbose:
                    print(
                        f"dirfiled: could not update field [{field}] due to {e.__class__}: {e}"
                    )
                pass
        ...
        if "derived_fields" in self.config:
            for field in self.config["derived_fields"]:
                ...
                if ftype == "lincom":
                    ...
                    self.df.add_linterp_entry(
                        field,
                        input_field=self.config["derived_fields"][field]["input_field"],
                        LUT_file=self.base_directory
                        + "/"
                        + self.config["derived_fields"][field]["LUT_file"],
                        units=self.config["derived_fields"][field]["units"],
                        label=self.config["derived_fields"][field]["label"],
                    )
...
class Main(QtCore.QObject):
    ...
    def __init__(
        ...
        self.dirfileWriter = DirfileWriter(
            base_directory, config, logger, ns_host, verbose=verbose
        )
