...
from housekeeping import data_handler, labjacks
...
class housekeeping:
    def __init__(
        ...
        if mode.lower() in ["m", "r"]:
            self.daq_telescope = data_handler.daq_loop(
                func=self.telescope.update_state,
                dt=self.hk_config["daq_dt"]["hk"],
        ...
        self.statedump_loop = data_handler.daq_loop(
            func=self.dump_state, dt=5000, name="state_dump"
        )
    ...
    def start_housekeeping_poll_loop(self):
        ...
        hk_poll_dt = int(np.round(self.hk_config["daq_dt"]["hk"], 0))
    ...
    def build_dicts(self):
        ...
        for rate in self.hk_config["daq_dt"]:
            # calculate the spf for each daq loop
            spf = int(
                self.hk_config["dirfile_write_dt"] / self.hk_config["daq_dt"][rate]
            )
            self.spf.update({rate: spf})
            print(f"{rate} daq loop: {spf} samples per frame")

        # go through all the fields
        for field in self.hk_config["fields"]:
