...
# ---------- ONLY COLOR/TRANSPARENCY CHANGES ----------
fig2.patch.set_alpha(0)
ax2.patch.set_alpha(0)

ax2.tick_params(which='both', colors='white')
...
for pos, count in zip(sorted_positions, sorted_counts):
    ...
    ax2.fill([ra1, ra2, ra3, ra4],
             [rad_dec - half_size * np.pi / 180,
              rad_dec - half_size * np.pi / 180,
              rad_dec + half_size * np.pi / 180,
              rad_dec + half_size * np.pi / 180],
...
ax2.set_xlabel('Right Ascension', color='white')
ax2.set_ylabel('Declination', color='white')

# Save as PNG for transparency (JPEG cannot be transparent)
figname = os.path.join(os.getenv("HOME"), 'data', 'skymap_cumulative.png')
fig2.savefig(figname, bbox_inches='tight', transparent=True, dpi=1200)
plt.close(fig2)
