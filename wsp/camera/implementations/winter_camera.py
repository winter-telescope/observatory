...
class local_camera(QtCore.QObject):
    ...
    def parse_state(self):
        ...
        # implement logic for deciding the camera state
        camera_state = self.assess_camera_state(self.state)
    ...
    # helper function to assess the camera state
    def assess_camera_state(self, state) -> CameraState:
        """
        Assess the camera state based on the current state dictionary.
        This is a simplified version of the logic that would be in the camera daemon.
        """
        if not state.get("is_connected", False):
            camera_state = CameraState.ERROR

        # STARTUP REQUESTED: startup is requested but not complete
        elif state.get("autoStartupRequested", False) and (
            state.get("autoStartComplete", False) is False
        ):
            camera_state = CameraState.STARTUP_REQUESTED

        # SETTING PARAMETERS: parameters are being set
        elif state.get("settingParameters", False):
            # for WINTER this is not implemented yet
            camera_state = CameraState.SETTING_PARAMETERS

        # READY: startup is complete and the camera is ready for exposure (not doing exposure or reading)
        elif state.get("autoStartComplete", False) and not state.get(
            "doing_exposure", False
        ):
            camera_state = CameraState.READY

        # EXPOSING: the camera is currently exposing an image
        elif state.get("doing_exposure", False):
            camera_state = CameraState.EXPOSING

        # ERROR: if none of the above, then we assume an error state
        else:
            camera_state = CameraState.ERROR
    ...
    def setDetbias(self, detbias, **kwargs):
        addrs = kwargs.get("addrs", None)
        self.remote_object.setDetbias(detbias, addrs=addrs)

    def tecSetCoeffs(self, Kp, Ki, Kd, **kwargs):
        addrs = kwargs.get("addrs", None)
        self.remote_object.tecSetCoeffs(Kp, Ki, Kd, addrs=addrs)

    def tecSetVolt(self, volt, **kwargs):
        addrs = kwargs.get("addrs", None)
