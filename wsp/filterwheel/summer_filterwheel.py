...
# -*- coding: utf-8 -*-
"""
Summer Filter Wheel Client

Local (WSP-process) client for the SUMMERfw Pyro daemon (summerFilterd.py).
Mirror of spring_filterwheel.py minus the shutter methods — the summer
wheel is a ZWO 7-position EFW with no shutter; darks use a blocked slot.
...
class local_filterwheel(QtCore.QObject):
    """
    Client for Summer filter wheel daemon
    """
    ...
    def parse_state(self):
        ...
        # note: the daemon publishes "is_connected" (spring's client reads a
        # nonexistent "connected" key here, which always booleans to True)
        self.state.update(
            {"is_connected": bool(self.remote_state.get("is_connected", False))}
        )
