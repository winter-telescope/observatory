...
# -*- coding: utf-8 -*-
"""
Summer Filter Wheel Daemon

Near-copy of springFilterd.py for the SUMMER camera's ZWO 7-position EFW,
which is plugged into the same raspberry pi as the spring FLI wheel and
shutter. Talks to the pi's web server /summer_filter_wheel endpoint
(wsp/viscam/web_server.py) and exposes the Pyro object "SUMMERfw".

Differences from spring:
- no shutter (the QHY42PRO/SUMMER path takes darks via a blocked wheel
  slot, config key filters.dark_filter, rather than a shutter)
- ZWO positions are 1-INDEXED slots 1..7, matching the labels printed on
  the physical wheel (the FLI/spring wheel is 0-indexed); poll is n=0
  (not n=8) so it can never collide with a slot number
- home = move to slot 1 (the EFW self-calibrates on power-up)
- the pi endpoint blocks until moves settle, mirroring the FLI
  endpoint's synchronous semantics
...
print(f"summerFilterd: wsp_path = {wsp_path}")

from daemon import daemon_utils
from utils import logging_setup


class SummerFilterWheel(QtCore.QObject):
    ...
    def __init__(self, config, logger=None, verbose=False):
        super(SummerFilterWheel, self).__init__()
        ...
        self.log("initing SummerFilterWheel object")
        ...
        self.endpoint = config["web_interface"].get(
            "endpoint", "/summer_filter_wheel"
        )
    ...
    def _fw_url(self, n):
        return f"{self.base_url}{self.endpoint}?n={n}"

    def pollStatus(self):
        """Get housekeeping status by querying the pi (poll sentinel is n=0)"""
        try:
            response = requests.get(self._fw_url(0), timeout=self.request_timeout)

            if response.status_code == 200:
                response_dict = response.json()
                self.parseResponse(response_dict)
                self.connected = True
                # fw_pos == -1 means the ZWO wheel is mid-move
                self.is_moving = 1 if self.filter_pos == -1 else 0
                self.update_state()
            else:
                if self.verbose:
    ...
    def home(self, verbose=False):
        """Home the filter wheel (lands on slot 1; ZWO slots are 1-indexed)"""
        ...
        try:
            response = requests.get(self._fw_url(-1), timeout=self.request_timeout)
            ...
            if response.status_code == 200:
                ...
                if self.filter_pos == 1 and self.fw_status == 1:
                    self.homed = 1
                    self.is_moving = 0
    ...
    def goToFilter(self, filter_num, verbose=False):
        ...
        ----------
        filter_num : int
            Filter position (1-indexed ZWO slot, 1-7, as labeled on the wheel)
        """
        ...
        try:
            # Move to filter position (pi endpoint blocks until settled)
            response = requests.get(
                self._fw_url(filter_num), timeout=self.request_timeout
            )
...
class SUMMERfw(QtCore.QObject):
    """Main filter wheel object exposed via Pyro"""

    newCmdRequest = QtCore.pyqtSignal(object)

    def __init__(self, config, logger=None, verbose=False):
        super(SUMMERfw, self).__init__()
...
class PyroGUI(QtCore.QObject):
    ...
    def __init__(self, config, ns_host=None, logger=None, verbose=False, parent=None):
        ...
        msg = f"(Thread {threading.get_ident()}: Starting up Summer Filter Daemon"
        if logger is None:
            print(msg)
        else:
            logger.info(msg)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"summerfilterd: arguments: {arguments}")
    ...
    print(f"summerfilterd: verbose = {verbose}")
    print(f"summerfilterd: logging mode = {doLogging}")

    # load the config
    fwconfig = os.path.join(wsp_path, "filterwheel", "summerfw_config.yaml")
