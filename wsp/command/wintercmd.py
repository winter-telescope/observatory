...
class Wintercmd(QtCore.QObject):
    ...
    def parse_list(self, cmdlist):
        ...
        try:
            ...
            self.logger.warning(
                f"Could not execute command list. Died at {cmd}, Error: {e}"
            )
    ...
    def defineParser(self, description=None):
        ...
        self.parser = ArgumentParser(
            logger=self.logger, description=description, usage=__doc__
        )
    ...
    def count(self):
        ...
        self.cmdparser.add_argument(
            "num", nargs=1, action=None, type=int, help="number to count up to"
        )
        self.getargs()
    ...
    def plover(self):
        ...
        self.cmdparser.add_argument(
            "num", nargs=1, action=None, type=int, help="integer number to plover"
        )
        self.getargs()
        num = self.args.num[0]
    ...
    def mount_set_slew_time_constant(self):
        ...
        self.cmdparser.add_argument(
            "tau", nargs=1, action=None, type=float, help="<tau_sec>"
        )
        self.getargs()
        tau = self.args.tau[0]
    ...
    def mount_goto_ra_dec_apparent(self):
        ...
        self.cmdparser.add_argument(
            "position", nargs=2, action=None, type=float, help="<ra_hours> <dec_degs>"
        )
        self.getargs()
        ...
        while True:
            ...
            stop_condition = (
                (not self.state["mount_is_slewing"])
                & (abs(self.state["mount_az_dist_to_target"]) < 0.1)
                & (abs(self.state["mount_alt_dist_to_target"]) < 0.1)
            )
            # do this in 2 steps. first shift the buffer forward (up to the last one. you end up with the last element twice)
            stop_condition_buffer[:-1] = stop_condition_buffer[1:]
            # now replace the last element
    ...
    def mount_goto_ra_dec_j2000(self):
        ...
        self.cmdparser.add_argument(
            "position", nargs=2, type=str, action=None, help="<ra_hours> <dec_degs>"
        )
        self.getargs()
        ra = self.args.position[0]
        dec = self.args.position[1]
        ...
        start_timestamp = datetime.utcnow().timestamp()
        self.logger.info(
            f"wintercmd: mount_goto_ra_dec_j2000 running in thread {threading.get_ident()}"
        )
        while True:
            QtCore.QCoreApplication.processEvents()
            # print('entering loop')
            ...
            stop_condition = (
                (not self.state["mount_is_slewing"])
                & (abs(self.state["mount_az_dist_to_target"]) < az_dist_lim)
                & (abs(self.state["mount_alt_dist_to_target"]) < alt_dist_lim)
            )
    ...
    def mount_goto_object(self):
        ...
        self.cmdparser.add_argument(
            "object_name", nargs=1, action=None, type=str, help="<object name>"
        )

        # ADD AN OPTIONAL PATH COMMAND.
        self.cmdparser.add_argument(
            "--force", "-f", action="store_true", help="<force move?>"
        )
        ...
        in_view = (local_alt_deg >= self.config["telescope"]["min_alt"]) & (
            local_alt_deg <= self.config["telescope"]["max_alt"]
        )
        msg_list = [
            f"wintercmd: request to point to object: {obj} at (RA, DEC) j2000= ({j2000_ra_hours:0.3f} h, {j2000_dec_deg:0.3f} deg)"
        ]
        msg_list.append(
            f'coords in catalog format (RA [h:m:s], DEC [d:am:as]) j2000 = ({j2000_coords.ra.to_string(u.hour, sep = ":")}, {j2000_coords.dec.to_string(u.deg, sep = ":")})'
        )
        msg_list.append(
            f"Current sky coords as viewed from Palomar: (Alt, Az) = ({local_alt_deg:0.3f} deg, {local_az_deg:0.3f} deg)"
        )
        if in_view:
            ...
            if force:
                msg_list.append(
                    "Object IS NOT in view, but you said to FORCE OBSERVATION so sending mount_goto_j2000 command"
                )
            else:
    ...
    def mount_dither_arcsec_radec(self):
        ...
        self.cmdparser.add_argument(
            "--relative", "-r", action="store_true", default=False, help="<force move?>"
        )
        self.getargs()
        ...
        if do_relative_offset:
            ...
            ra_center_j2000_hours = ra0_j2000_hours - self.state[
                "mount_offsets_ra_arcsec_total"
            ] * (1 / 3600) * (24 / 360)
            dec_center_j2000_deg = dec0_j2000_deg - self.state[
                "mount_offsets_dec_arcsec_total"
            ] * (1 / 3600)

        start = astropy.coordinates.SkyCoord(
            ra=ra_center_j2000_hours * u.hour, dec=dec_center_j2000_deg * u.deg
        )
        ...
        if do_relative_offset:
            ...
            ra_delta_arcsec_from_current = (
                ra_delta_arcsec - self.state["mount_offsets_ra_arcsec_total"]
            )
            dec_delta_arcsec_from_current = (
                dec_delta_arcsec - self.state["mount_offsets_dec_arcsec_total"]
            )
        ...
        obstime = astropy.time.Time(obstime_mjd, format="mjd", location=self.ephem.site)
        ...
        if True:
            if do_relative_offset:
                self.logger.info(
                    "executing RELATIVE dither with respect to current mount position"
                )
            else:
                self.logger.info(
                    "executing ABSOLUTE dither with respect to the zero-offset mount position"
                )
            self.logger.info(
                f"executing dither: RA Dist = {ra_dist_arcsec:.6f}, Dec Dist = {dec_dist_arcsec:.6f}"
            )
            self.logger.info(
                f"executing dither: RA Dist = {ra_dist_arcsec:.6f}, Dec Dist = {dec_dist_arcsec:.6f}"
            )
            ...
            self.logger.info(
                f"ra delta   = {ra_delta_arcsec:>10.6f} arcsec ({ra_delta_arcsec/60.0:>6.3f} arcmin)"
            )
            self.logger.info(
                f"dec delta = {dec_delta_arcsec:>10.6f} arcsec ({dec_delta_arcsec/60.0:>6.3f} arcmin)"
            )

            self.logger.info(f"Alt/Az Coords: Current --> Finish")
            self.logger.info(
                f'Alt : {self.state["mount_alt_deg"]:>10.6f} --> {goal_alt:>10.6f} (deg)'
            )
            self.logger.info(
                f'Az : {self.state["mount_az_deg"]:>10.6f} --> {goal_az:>10.6f} (deg)'
            )

            self.logger.info(f"RA/Dec Coords: Start --> Finish")
            self.logger.info(
                f"ra : {start.ra.hour:>10.6f} --> {end.ra.hour:>10.6f} (hour)"
            )
            self.logger.info(
                f"dec: {start.dec.deg:>10.6f} --> {end.dec.deg:>10.6f} (deg)"
            )
        ...
        if dec_dist_arcsec != 0.0:
            self.parse(
                f"mount_offset dec add_arcsec {dec_delta_arcsec_from_current:.3f}"
            )
    ...
    def mount_random_dither_arcsec(self):
        ...
        self.defineCmdParser(
            "execute a random dither from within a ra-dec circle of radius <step> arcseconds"
        )
        ...
        if minradius >= radius:
            self.logger.info(
                f"min radius {minradius} is >= radius {radius}. setting to minradius to zero"
            )
    ...
    def mount_offset_arcsec(self):
        ...
        self.defineCmdParser(
            "dither the mount in the specified axis by the specified arcminutes"
        )
        self.cmdparser.add_argument(
            "axis",
            nargs=1,
        ...
        self.cmdparser.add_argument(
            "arcsec", nargs=1, type=float, action=None, help="arcsec to dither"
        )
        ...
        while True:
            ...
            dist_to_target_low = (
                (not self.state["mount_is_slewing"])
                & (abs(self.state["mount_az_dist_to_target"]) < threshold_arcsec)
                & (abs(self.state["mount_alt_dist_to_target"]) < threshold_arcsec)
            )

            ra_dist_hours = abs(
                self.state["mount_ra_j2000_hours"] - ra_j2000_hours_goal
            )
            ra_dist_arcsec = ra_dist_hours * (360 / 24.0) * 3600.0
            # print(f'ra dist to target = {ra_dist_arcsec:.2f} arcsec')
            ra_in_range = ra_dist_arcsec < threshold_arcsec
    ...
    def mount_dither(self):
        ...
        self.cmdparser.add_argument(
            "arcmin", nargs=1, type=float, action=None, help="arcminutes to dither"
        )
        ...
        if axis == "ra":
            arcmin = arcmin * (
                1 / 0.0667
            )  # this is a stupid thing we have to do because the ra offset doesn't work right
        else:
            pass
        ...
        if arcmin > max_arcmin:
            self.logger.warning(
                f"wintercmd: MAX DITHER DISTANCE IS {max_arcmin} ARCMIN. RETURNING..."
            )
            return
        ...
        while True:
            ...
            ra_in_range = (
                abs(self.state["mount_ra_j2000_hours"]) - ra_j2000_hours_goal
            ) < threshold_hours
            dec_in_range = (
                abs(self.state["mount_dec_j2000_deg"]) - dec_j2000_deg_goal
            ) < threshold_arcmin
    ...
    def mount_offset(self):
        ...
        self.cmdparser.add_argument("offset", nargs=3, action=None, help=helptxt)

        self.getargs()
        # all args are strings
        axis = self.args.offset[0].lower()
        action = self.args.offset[1].lower()
        value = float(self.args.offset[2])
    ...
    def mount_goto_alt_az(self):
        ...
        self.cmdparser.add_argument(
            "position", nargs=2, action=None, type=float, help="<alt_deg> <az_deg>"
        )
        self.getargs()
        ...
        while True:
            ...
            dist = (
                (alt - self.state["mount_alt_deg"]) ** 2
                + (az - self.state["mount_az_deg"]) ** 2
            ) ** 0.5
            """
            self.logger.info(f'wintercmd (thread {threading.get_ident()}: count = {self.state["count"]}')
            self.logger.info(f'wintercmd (thread {threading.get_ident()}: mount_is_slewing: {self.state["mount_is_slewing"]}')
    ...
    def mount_follow_tle(self):
        ...
        """
        self.defineCmdParser(
            "track a two line element (TLE) using the standard NORAD format "
        )
        self.cmdparser.add_argument(
            "tle",
            nargs="+",
    ...
    def mount_model_save(self):
        ...
        self.cmdparser.add_argument("filename", nargs=1, action=None, help="<filepath>")
    ...
    def doFocusLoop(self):
        ...
        print(
            f"running focus loop with center = {center}, throw = {throw}, nsteps = {nsteps}"
        )

        sigcmd = signalCmd(
            "do_focusLoop", nom_focus=center, total_throw=throw, nsteps=nsteps
        )
    ...
    def doFocusLoop_old(self):
        ...
        self.defineCmdParser(
            "retrieve whether or not a plot will be displayed based on img fwhm"
        )
        self.cmdparser.add_argument(
            "--noplot", action="store_true", default=False, help="<position_steps>"
        )
        self.cmdparser.add_argument(
            "--fine", action="store_true", default=False, help="<position_steps>"
        )
        self.cmdparser.add_argument(
            "--roborun",
            action="store_true",
        ...
        if self.args.roborun:
            roborun = True
            self.logger.info(
                "wintercmd: doFocusLoop: I will run the robotic schedule when I am finished!"
            )
        else:
            roborun = False
        ...
        if self.args.roboruntest:
            roboruntest = True
            self.logger.info(
                "wintercmd: doFocusLoop: I will run the robotic schedule IN TEST MODE when I am finished!"
            )
        else:
            roboruntest = False
        ...
        loop = summerFocusLoop.Focus_loop(
            filt_numlist[current_filter], self.config, fine
        )
        ...
        try:
            ...
            df.to_csv(
                image_log_path
                + "focusLoop"
                + self.state["mount_timestamp_utc"]
                + ".csv"
            )
        ...
        try:
            ...
            self.logger.info(
                "Focuser_going to final position at %s microns" % (focuser_pos)
            )
        ...
        try:
            if plotting:
                ...
                alert_config = yaml.load(
                    open(alert_config_file), Loader=yaml.FullLoader
                )
    ...
    def m2_focuser_enable(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to enable M2 focuser: command timed out after {timeout} seconds before completing."
                )
    ...
    def m2_focuser_disable(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to disable M2 focuser: command timed out after {timeout} seconds before completing."
                )
    ...
    def m2_focuser_goto(self):
        ...
        self.cmdparser.add_argument(
            "position", nargs=1, action=None, type=float, help="<position_steps>"
        )
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f'unable to goto M2 focuser position: requested pos = {target}, actual pos = {self.state["focuser_position"]}, command timed out after {timeout} seconds before completing.'
                )
    ...
    def rotator_goto_mech(self):
        ...
        self.logger.info(
            f"wintercmd rotator_goto_mech (thread {threading.get_ident()}): target = {target}, type(target) = {type(target)}"
        )
        ...
        while True:
            ...
            stop_condition = (self.state["rotator_is_slewing"] == False) & (
                np.abs(self.state["rotator_mech_position"] - target) < 0.05
            )
            # do this in 2 steps. first shift the buffer forward (up to the last one. you end up with the last element twice)
            stop_condition_buffer[:-1] = stop_condition_buffer[1:]
            # now replace the last element
    ...
    def dome_home(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"dome never started homing! waited {timeout} seconds"
                )

            stop_condition = (
                self.state["dome_status"]
                == self.config["Dome_Status_Dict"]["Dome_Status"]["HOMING"]
            )
            # do this in 2 steps. first shift the buffer forward (up to the last one. you end up with the last element twice)
            stop_condition_buffer[:-1] = stop_condition_buffer[1:]
            # now replace the last element
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"dome homing command timed out after {timeout} seconds before completing"
                )

            stop_condition = (self.dome.Home_Status == "READY") & (
                self.dome.Dome_Status == "STOPPED"
            )
            # do this in 2 steps. first shift the buffer forward (up to the last one. you end up with the last element twice)
            stop_condition_buffer[:-1] = stop_condition_buffer[1:]
            # now replace the last element
    ...
    def dome_goto(self):
        ...
        self.cmdparser.add_argument(
            "azimuth", nargs=1, action=None, help="<azimuth_degs>"
        )
        ...
        drivetime = (
            np.abs(dist_to_go) / effective_speed + lag_time
        )  # total time to move
        ...
        while True:
            ...
            stop_condition = (
                self.state["dome_status"]
                == self.config["Dome_Status_Dict"]["Dome_Status"]["STOPPED"]
            ) and (np.abs(self.state["dome_az_deg"] - az) < 0.5)
            # do this in 2 steps. first shift the buffer forward (up to the last one. you end up with the last element twice)
            stop_condition_buffer[:-1] = stop_condition_buffer[1:]
            # now replace the last element
            stop_condition_buffer[-1] = stop_condition
    ...
    def dome_tracking_on(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to enable dome tracking: command timed out after {timeout} seconds before completing."
                )
    ...
    def dome_tracking_off(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to disable dome tracking: command timed out after {timeout} seconds before completing."
                )
    ...
    def chiller_set_setpoint(self):
        ...
        self.cmdparser.add_argument(
            "temperature", nargs=1, action=None, help="<setpoint_celsius>"
        )

        self.getargs()
        temp = float(self.args.temperature[0])  # remember the args come in as strings!!
    ...
    def chiller_write_register(self):
        ...
        self.cmdparser.add_argument(
            "request", nargs=2, action=None, help="<regname> <value>"
        )
        self.getargs()
        regname = self.args.request[0]
        val = float(self.args.request[1])
    ...
    def pdu_off(self):
        ...
        if pdu_num in active_pdus:
            ...
            self.logger.info(
                f"right now PDU numbers are hardcoded, and only {active_pdus} are active"
            )
    ...
    def pdu_cycle(self):
        ...
        self.cmdparser.add_argument(
            "channel", nargs=2, type=int, action=None, help="<pdu_num> <chan_num>"
        )
        self.getargs()
    ...
    def do_routine(self):
        ...
        """
        self.defineCmdParser(
            "execute routine from specified text file. default path is wsp/routines"
        )
        self.cmdparser.add_argument("filename", nargs=1, action=None, help="<filename>")

        # ADD AN OPTIONAL PATH COMMAND.
        self.cmdparser.add_argument("--path", nargs=1, action=None, help="<filepath>")
    ...
    def robo_do_calibration(self):
        ...
        self.cmdparser.add_argument(
            "-df", "--domeflat", action="store_true", default=False
        )
        self.getargs()
        ...
        self.logger.info(
            f"wintercmd: running roboOperator do_calibration with do_flats = {do_flats}, do_darks = {do_darks}, do_bias = {do_bias}"
        )
    ...
    def robo_do_exposure(self):
        self.defineCmdParser(
            "tell the robotic operator to take an image with the camera"
        )
        ...
        self.cmdparser.add_argument(
            "--noplot", action="store_true", default=False, help="<comment> "
        )
        ...
        sigcmd = signalCmd(
            "doExposure", obstype=obstype, postPlot=postPlot, qcomment=comment
        )
    ...
    def robo_take_flat(self):
        ...
        sigcmd = signalCmd(
            "doExposure", obstype="FLAT", postPlot=True, qcomment="altaz"
        )
    ...
    def robo_observe(self):
        ...
        # argument to hold the coordinates/location of the target
        self.cmdparser.add_argument(
            "target", action=None, type=str, nargs="*", help="<target> {<target>}"
        )
        ...
        obsmode_group.add_argument(
            "-man", "--manual", action="store_true", default=False
        )
        obsmode_group.add_argument("--schedule", action="store_true", default=False)
        obsmode_group.add_argument(
            "-cal", "--calibration", action="store_true", default=False
        )
    ...
    def robo_observe_altaz(self):
        """Usage: mount_goto_alt_az <alt> <az>"""
        self.defineCmdParser(
            "tell the robotic operator to execute on observation of the specified alt and az"
        )
        self.cmdparser.add_argument(
            "position", nargs=2, action=None, type=float, help="<alt_deg> <az_deg>"
        )
    ...
    def robo_set_operator(self):
        ...
        self.cmdparser.add_argument(
            "operator_name", nargs=1, action=None, type=str, help="<operator name>"
        )

        self.getargs()
        name = self.args.operator_name[0]
        sigcmd = signalCmd("updateOperator", operator_name=name)
    ...
    def robo_set_qcomment(self):
        self.defineCmdParser(
            "set comment which will be written to QCOMMENT in the fits header"
        )
        self.cmdparser.add_argument(
            "qcomment", nargs=1, action=None, type=str, help="<queue comment>"
        )

        self.getargs()
        qcomment = self.args.qcomment[0]
        sigcmd = signalCmd("updateQComment", qcomment=qcomment)
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f'command timed out after {timeout} seconds before completing. Requested qcomment = {qcomment}, but it is {self.state["qcomment"]}'
                )
            ...
            if all(entry == condition for entry in stop_condition_buffer):
                self.logger.info(
                    f'wintercmd: qcomment set successfully. Current qcomment = {self.state["qcomment"]}'
                )
                break
    ...
    def robo_set_obstype(self):
        self.defineCmdParser(
            "set the current observation type (flat, dark, bias, science, etc"
        )
        self.cmdparser.add_argument(
            "obstype", nargs=1, action=None, type=str, help="<observation type>"
        )

        self.getargs()
        obstype = self.args.obstype[0]
        sigcmd = signalCmd("updateObsType", obstype=obstype)
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f'command timed out after {timeout} seconds before completing. Requested obstype = {obstype}, but it is {self.state["obstype"]}'
                )
            ...
            if all(entry == condition for entry in stop_condition_buffer):
                self.logger.info(
                    f'wintercmd: obstype set successfully. Current obstype = {self.state["obstype"]}'
                )
                break
    ...
    def mirror_cover_connect(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to connect to mirror cover: command timed out after {timeout} seconds before completing."
                )
    ...
    def mirror_cover_open(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to open mirror cover: command timed out after {timeout} seconds before completing."
                )

            stop_condition = (self.state["Mirror_Cover_State"] == 0) and (
                self.state["Mirror_Cover_Connected"]
            )
            # do this in 2 steps. first shift the buffer forward (up to the last one. you end up with the last element twice)
            stop_condition_buffer[:-1] = stop_condition_buffer[1:]
            # now replace the last element
    ...
    def mirror_cover_close(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to close mirror cover: command timed out after {timeout} seconds before completing."
                )

            stop_condition = (self.state["Mirror_Cover_State"] == 1) and (
                self.state["Mirror_Cover_Connected"]
            )
            # do this in 2 steps. first shift the buffer forward (up to the last one. you end up with the last element twice)
            stop_condition_buffer[:-1] = stop_condition_buffer[1:]
    ...
    def command_viscam_shutter(self):
        ...
        self.cmdparser.add_argument(
            "shutter_cmd", nargs=1, action=None, help="<shutter_int>"
        )
    ...
    def command_filter_wheel(self):
        ...
        self.cmdparser.add_argument(
            "fw_pos", nargs=1, type=int, action=None, help="<fw_pos_int>"
        )

        self.getargs()
        fw_pos = self.args.fw_pos[0]

        sigcmd = signalCmd("command_filter_wheel", pos=fw_pos)
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to move viscam filter wheel: command timed out after {timeout} seconds before completing."
                )
            ...
            if all(entry == condition for entry in stop_condition_buffer):
                self.logger.info(
                    f"wintercmd: successfully completed viscam filter wheel move"
                )
                break
    ...
    def ccd_set_exposure(self):
        ...
        self.cmdparser.add_argument(
            "seconds", nargs=1, action=None, help="<exposure_time_seconds>"
        )

        self.getargs()
        secs = float(self.args.seconds[0])  # remember the args come in as strings!!
        ...
        while True:
            ...
            if dt > timeout:
                self.logger.info(
                    f'command timed out after {timeout} seconds before completing. Requested exptime = {secs}, but it is {self.state["ccd_exptime"]}'
                )
                break
            ...
            if all(entry == condition for entry in stop_condition_buffer):
                self.logger.info(
                    f'wintercmd: ccd_exptime set successfully. Current Exptime = {self.state["ccd_exptime"]}'
                )
                try_again = False
                break
            try_again = True
    ...
    def ccd_set_tec_sp(self):
        ...
        self.cmdparser.add_argument(
            "degrees", nargs=1, action=None, help="<tec_sp_celsius>"
        )

        self.getargs()
        degs = float(self.args.degrees[0])  # remember the args come in as strings!!
    ...
    def ccd_do_exposure(self):
        ...
        sigcmd = signalCmd("doExposure", dark=dark)

        self.ccd.newCommand.emit(sigcmd)

        self.logger.info(
            f"wintercmd: running ccd_do_exposure in thread {threading.get_ident()}"
        )
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"ccd_do_exposure command timed out after {timeout} seconds before completing"
                )

            stop_condition = (self.state["ccd_doing_exposure"] == False) & (
                self.state["ccd_image_saved_flag"]
            )
            ...
            if all(entry == condition for entry in stop_condition_buffer):
                self.logger.info(
                    f"wintercmd: finished the do exposure method without timing out :)"
                )
                break
    ...
    def fw_goto(self):
        ...
        self.cmdparser.add_argument(
            "pos", nargs=1, action=None, type=int, help="<position_number>"
        )
    ...
    def doExposure(self):
        ...
        self.cmdparser.add_argument(
            "--mode", nargs=1, action=None, type=str, default="", help="<image_mode>"
        )
        ...
        # add ability to pass in sensor addresses
        self.cmdparser.add_argument(
            "--imname", nargs=1, action=None, type=str, default="", help="<image_name>"
        )
        ...
        self.logger.info(
            f"wintercmd: imtype = {imtype}, mode = {mode}, imdir = {imdir}, addrs = {addrs}"
        )
        ...
        while True:
            ...
            if all(entry == condition for entry in stop_condition_buffer):
                self.logger.info(
                    f"wintercmd: finished the doExposure method without timing out :)"
                )
                break
    ...
    def tecSetVolt(self):
        ...
        self.cmdparser.add_argument(
            "voltage", nargs=1, action=None, type=float, help="<tec_voltage>"
        )
        ...
        self.logger.info(
            f"wintercmd: setting TEC voltage on {camera.daemonname} to {voltage} V"
        )
    ...
    def setExposure(self):
        ...
        self.cmdparser.add_argument(
            "exptime", nargs=1, action=None, type=float, help="<exptime_seconds>"
        )
        ...
        while True:
            ...
            if dt > timeout:
                self.logger.info(
                    f'command timed out after {timeout} seconds before completing. Requested exptime = {exptime}, but it is {self.state["ccd_exptime"]}'
                )
                break
            ...
            if all(entry == condition for entry in stop_condition_buffer):
                ...
                self.logger.info(
                    f"wintercmd: {camname} camera exptime set successfully."
                )
                try_again = False
                break
            try_again = True
    ...
    def updateSensorValidation(self):
        ...
        self.cmdparser.add_argument(
            "validatation", nargs=1, action=None, type=str2bool, help="<sensor_okay>"
        )
        ...
        self.logger.info(
            f"wintercmd: setting sensor validation voltage on {camera.daemonname}: {addrs} to {validatation} "
        )
    ...
    def generate_supernovae_db(self):
        ...
        df.to_sql(
            "Summary", con=connection, if_exists="replace", index_label="obsHistID"
        )
        df_2 = pd.DataFrame.from_dict({"Nonsense that is required": [1, 2, 3, 4, 5]})
        df_2.to_sql(
            "Fields", con=connection, if_exists="replace", index_label="fieldID"
        )
        print("Finished")
    ...
    def do_startup(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to run startup: command timed out after {timeout} seconds before completing."
                )
            conds = []
            ...
            conds.append(
                np.abs(
                    self.state["mount_alt_deg"]
                    - self.config["telescope"]["home_alt_degs"]
                )
                < 1.0
            )  # home is 45 deg, so this isn't really doing anything
            conds.append(
                np.abs(
    ...
    def total_shutdown(self):
        ...
        while True:
            ...
            if dt > timeout:
                raise TimeoutError(
                    f"unable to connect to shut down observatory: command timed out after {timeout} seconds before completing."
                )
