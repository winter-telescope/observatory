...
class CalTracker(object):
    def __init__(self, config, active_cams = ['winter'], logger = None, sunsim = False, verbose = False):
        ...
        self.triggerhandler.setupTrigLog(logpath = self.cal_log_filepath, linkpath = self.cal_log_linkpath)
    ...
    def getCalsToDo(self, sun_alt:float, timestamp:float, sun_rising:bool)->list:
        ...
        cals_to_do = self.triggerhandler.getActiveTriggers(sun_alt, timestamp, sun_rising)
    ...
    def logCommand(self, trigname, sun_alt, timestamp):
        ...
        self.log(f'logging that we attempted to handle cal trigger {trigname} at sun_alt = {sun_alt}, timestamp = {timestamp}')
