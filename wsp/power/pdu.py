...
class PDU(object):
    ...
    def _digitalLoggersSend(self, command):
        ...
        if self.brand.lower() == "digital loggers":
            try:
                ...
                status_code = response.text[
                    status_key_index
                    + len(status_key) : status_key_index
                    + len(status_key)
                    + 2
                ]
                ...
                status_str = np.append(
                    np.flip(list(status_a_bin)), np.flip(list(status_b_bin))
                )
    ...
    def getOutletNames(self):
        ...
        if self.brand.lower() != "digital loggers":
            ...
            # get information from all outlets
            ...
            raise IOError(f"Unknown PDU brand: {self.brand}")
    ...
    # method to look up the outlet number by name or number
    def lookupOutlet(self, outlet):
        # look up the outlet number
        if type(outlet) is int:
            ...
            # if the outlet name is "EMPTY" then we don't do anything
            if outlet.lower() == "empty":
                self.log("Cannot look up outlet by name 'EMPTY'. Returning.")
                return

            # if it's a proper name look it up in the reverse dict
    ...
    def cycle(self, outlet):
        ...
        outletnum = self.lookupOutlet(outlet)
        if outletnum is None:
            return

        # now send the cycle command
        if self.brand.lower() == "digital loggers":

            command = "outlet?" + str(outletnum) + "=CCL"
            self.log(f"Sending CYCLE Command to Outlet # {outletnum}")
            self._digitalLoggersSend(command)

        elif self.brand.lower() == "synaccess":
            self.log(f"Sending CYCLE Command to Outlet # {outletnum}")
            self.synlinkpdu.outlets.set_state(outletnum, "REBOOT")
    ...
    def on(self, outlet):
        ...
        if self.brand.lower() == "digital loggers":
            ...
            self.log(f"Sending ON Command to Outlet # {outletnum}")
            self.synlinkpdu.outlets.set_state(outletnum, "ON")
    ...
    def off(self, outlet):
        ...
        if self.brand.lower() == "digital loggers":
            ...
            self.log(f"Sending OFF Command to Outlet # {outletnum}")
            self.synlinkpdu.outlets.set_state(outletnum, "OFF")
    ...
    def sendStatus(self, status_arr):
        ...
        if len(status_arr) != self.num_outlets:
            raise IOError(
                "The number of status bits in your command do not match the number of outlets"
            )
        ...
        for state in status_arr:
            if state not in [1, 0, 2]:
                raise IOError(
                    "Outlet States must be 1 (on) or 0 (off) or 2 (do nothing)!"
                )
