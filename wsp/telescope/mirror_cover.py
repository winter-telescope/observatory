...
class MirrorCovers:
    ...
    def open_mirror_cover_socket(self, addr, port):
        ...
        self.logger.info(
            f"MirrorCover: Attempting to connect to PWShutter TCP server at address {addr} and port {port}"
        )
        ...
        try:
            ...
            self.connected = True
            self.logger.info(
                f"MirrorCover: Successfully opened socket connection to mirror cover!"
            )
        except:
            self.connected = False
    ...
    def update_state(self):
        ...
        if self.connected:
            ...
            if (
                ...
                self.state.update(
                    {"mirror_cover_connected_last_timestamp": now_timestamp}
                )
                self.state.update(
                    {
                        "mirror_cover_state_str": MIRROR_COVER_STATUS_DICT.get(
                            code, "ERROR"
                        )
                    }
                )
                ...
                self.state.update({"mirror_cover_state": code})
                self.state.update({"mirror_cover_state_str": "ERROR"})
                self.state.update({"mirror_cover_connected": self.connected})
        else:
            # Handle the case when not connected
            self.state.update(
                {
                    "mirror_cover_connected": False,
                    "mirror_cover_connected_last_timestamp": now_timestamp,
                    "mirror_cover_state": 255,
                    "mirror_cover_state_str": "ERROR",
                }
            )
