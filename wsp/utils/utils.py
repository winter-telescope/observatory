...
def offset_coordinates_to_detector_position(
    ...
    x_offset_pixels = (
        (requested_board_x - base_board_x) * winter_x_pixels
        + x_pixel
        - base_pointing_x_pixel
    )
    y_offset_pixels = (
        (requested_board_y - base_board_y) * winter_y_pixels
        + y_pixel
...
def getFromFITSHeader(filename, keyword):
    try:
        ...
        value = None
        print(
            f"could not get keyword {keyword} from file {filename} due to {type(e)}: {e}"
        )
    return value
...
def getImages(starttime, endtime, camera="SUMMER", fullpath=True):
    ...
    if type(starttime) is str:
        ...
        raise TypeError(
            "starttime must be either a datetime object, string in YYYMMDD_HHMMSS, or MJD"
        )
    ...
    if type(endtime) is str:
        ...
        raise TypeError(
            "endtime must be either a datetime object, string in YYYMMDD_HHMMSS, or MJD"
        )
    ...
    for file in filelist:
        if camera.lower() == "summer":
            ...
            raise TypeError(
                "getImages can only search for SUMMER images, other cameras not implemented"
            )
...
def astrometry(
    ...
    rakey="RA",
    deckey="DEC",
    pixscalekey="PIXSCALE",
    pixscale=None,
    nopositionlimit=False,
    noquadlimit=False,
):
    ...
    try:
        ...
        ra = ten(hdr[rakey]) * 15.0

    try:
        dec = float(hdr[deckey])
    except:
        dec = ten(hdr[deckey])
    if dec > 90.0:
        dec = dec - 360.0
    ...
    if not noquadlimit:
        cmd += " --quad-size-min 0.4" + " --quad-size-max 0.6"
    cmd += (
        " --cpulimit 30"
        + " --no-verify"
        + " --crpix-center"
    ...
    with open(catname, "rb") as filep:
        ...
        for line in filep:
            ...
            # older sextractor catalogs contain a few nuisance lines; ignore those
                ...
                not line.startswith("-----")
                and not line.startswith("Measuring")
                and not line.startswith("(M+D)")
                and not line.startswith("Objects:")
            ):
...
def findBrightest(imageName):
    ...
    try:
        brightest = np.argmax(cat["FLUX_ISO"])
    except:
        return None, None
...
if __name__ == "__main__":
    ...
    d = query_server("status?", socket=sock, end_char="}", timeout=1)
    print(d)
