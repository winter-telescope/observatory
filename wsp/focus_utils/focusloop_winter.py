...
def run_sextractor(
    ...
    try:
        command = (
            f"sex -c {astrom_sex} {imgname} -CATALOG_NAME {imgname}.cat "
...
def get_img_fwhm(
    ...
    assert len(imgnames) == len(weightimages), (
        "Number of images and weight images " "must be the same."
    )
    full_img_cat = Table()
...
def fit_parabola(
    ...
    if plot:
        ...
        ax.text(
            0.5,
            0.9,
            "Best FWHM : %.1f arcsec, focus = %i, Fit Min Focus = %.1f"
...
def analyse_imgs_focus(
    imglists: list[list[str]],
    pixscale=1.00,
    exclude: bool = False,
    focus_keyword="FOCPOS",
    maskdir: str = None,
):
    """
    ...
    for imglist in imglists:
        ...
        mean, med, std = get_img_fwhm(
            imglist, pixscale=pixscale, exclude=exclude, weightimages=masknames
        )
...
def save_split_images(
    ...
    with fits.open(filename) as hdu:
        ...
        for ext in range(1, num_ext):
            ...
            splitfile_basename = (
                f"{os.path.basename(filename).replace('.fits', '')}_"
...
def split_and_calibrate_images(
    imglist: list[str],
    board_ids: list = [0],
...
def subtract_sky(imagelist: [list[list[str]]]) -> list[list[str]]:
    ...
    for boardid in range(imagelist.shape[1]):
        ...
        for count, image in enumerate(board_images):
            ...
            fits.writeto(
                sky_sub_path, sky_sub_data[count], fits.getheader(image), overwrite=True
            )
...
def calculate_best_focus_from_images(
    image_dir: str | Path,
    masterdarks_dir: str | Path = MASTERDARK_DIR,
    masterflats_dir: str | Path = MASTERFLAT_DIR,
    ...
    if not skip_calibrate:
        ...
        split_paths_list = split_and_calibrate_images(
            imagelist,
    ...
    med_fwhms, std_fwhms, focus_vals = analyse_imgs_focus(
        split_paths_list, maskdir=maskdir
    )
    ...
    with open(statsfile, "w") as f:
        ...
        for img, fwhm, fwhm_std, focus in zip(
            sorted_split_paths_list, med_fwhms, std_fwhms, focus_vals
        ):
...
def plot_all_detectors_focus(files_dir: str | Path):
    ...
    for ind in range(6):
        focus = pd.read_csv(f"{files_dir}/focusloop_stats_{ind}.txt")
        focus["BOARDID"] = ind
...
def run(cmd_args) -> float:
    ...
    matplotlib_init()
    best_focus = calculate_best_focus_from_images(
        cmd_args.image_dir,
        ...
        statsfile=os.path.join(cmd_args.output_dir, "focusloop_stats.txt"),
    )

    if cmd_args.plot_all:
        for board_id in range(6):
            _ = calculate_best_focus_from_images(
                cmd_args.image_dir,
                masterdarks_dir=cmd_args.masterdarks_dir,
                masterflats_dir=cmd_args.masterflats_dir,
                maskdir=cmd_args.masks_dir,
                ...
                statsfile=os.path.join(
                    cmd_args.output_dir, f"focusloop_stats_{board_id}" f".txt"
                ),
                plot=False,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--masterdarks_dir", type=str, default=MASTERDARK_DIR)
    parser.add_argument("--masterflats_dir", type=str, default=MASTERFLAT_DIR)
    parser.add_argument("--masks_dir", type=str, default=MASK_DIR)
