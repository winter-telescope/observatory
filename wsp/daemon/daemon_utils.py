...
class PyroGUI(QtCore.QObject):
    ...
    def __init__(self, Object, name, ns_host=None, daemon_host=None, parent=None):
        ...
        self.pyro_thread = PyroDaemon(
            obj=self.Object, name=name, ns_host=ns_host, daemon_host=daemon_host
        )
    ...
    def launch_all(self):
        for key in self.daemons.keys():
            try:
                ...
                print(
                    f"launching {key} daemon in system PID {self.daemons[key].process.pid}"
                )
...
def cleanup(daemons_to_kill=list(), logger=None):
    ...
    for p in py_processes:
        try:
            if any(
                daemon_to_kill in p.cmdline()[1] for daemon_to_kill in daemons_to_kill
            ):
    ...
    if printall:
        ...
        for p in py_processes:
            try:
                ...
                print(
                    f'\t\t\tParent Process = {p.parent().cmdline()[1].split("/")[-1]}'
                )
                print()
            except:
                pass
