...
class StatusMonitor(QtCore.QObject):
    ...
    def pollStatus(self):
        ...
        if self.connected:
            ...
            try:
                dome_state = utils.query_socket(
                    self.sock, "status?", end_char="}", timeout=2
                )
...
class CommandHandler(QtCore.QObject):
    ...
    def connect_socket(self):
        if self.verbose:
            self.log(
                f"(Thread {threading.get_ident()}) CommandHandler: Attempting to connect socket"
            )
        # record the time of this connection attempt
        # self.reset_last_recconnect_timestamp()
        # self.reconnector.reset_last_reconnect_timestamp()
        ...
        try:
            ...
            if self.verbose:
                self.log(
                    f"(Thread {threading.get_ident()}) Connection attempt successful!"
                )
            ...
            if self.verbose:
                self.log(
                    f"(Thread {threading.get_ident()}) CommandHandler: connection unsuccessful."
                )
        ...
        if self.connected:
            ...
            try:
                ...
                if "godome" in cmd:
                    ...
                    self.log(
                        f"CommandHandler: Sent command {cmd} to dome. Received reply: {reply}"
                    )
            ...
            self.log(
                f"CommandHandler: Received command: {cmd}, but dome was disconnected. Reply = {self.disconnectedReply}"
            )
...
class PyroGUI(QtCore.QObject):
    ...
    def __init__(
        ...
        self.pyro_thread = daemon_utils.PyroDaemon(
            obj=self.dome, name="dome", ns_host=ns_host
        )
...
if __name__ == "__main__":
    ...
    print(f"launching dome daemon with ns_host = {ns_host}, domesim = {domesim}")
    ...
    main = PyroGUI(
        config=config, ns_host=ns_host, logger=logger, verbose=verbose, domesim=domesim
    )
