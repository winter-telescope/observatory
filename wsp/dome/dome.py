...
class local_dome(QtCore.QObject):
    ...
    def __init__(
        self, base_directory, config, ns_host=None, logger=None, telescope=None
    ):
        ...
        utc_datetime_obj = datetime.strptime(
            default_timestr, "%Y-%m-%d %H:%M:%S.%f"
        )  # last query time string
    ...
    def parse_state(
        ...
        self.state.update(
            {"last_command_reply": self.remote_state.get("command_reply", self.default)}
        )
        self.state.update(
            {"query_timestamp": self.remote_state.get("timestamp", self.default)}
        )
        self.state.update(
            {
                "reconnect_remaining_time": self.remote_state.get(
                    "reconnect_remaining_time", self.default
                )
            }
        )
        self.state.update(
            {
                "reconnect_timeout": self.remote_state.get(
                    "reconnect_timeout", self.default
                )
            }
        )
        self.state.update(
            {"is_connected": bool(self.remote_state.get("is_connected", self.default))}
        )

        # timestamp of last reply
        utc = self.remote_state.get(
        ...
        self.state.update(
            {
                "Telescope_Power": self.config["Dome_Status_Dict"][
                    "Telescope_Power"
                ].get(self.Telescope_Power, self.default)
            }
        )
        # self.state.update({'Telescope_Power'                :   self.remote_state.get('Telescope_Power',                  self.default)})

        self.state.update(
            {"Dome_Azimuth": self.remote_state.get("Dome_Azimuth", self.default)}
        )

        self.Dome_Status = self.remote_state.get(
            "Dome_Status", "FAULT"
        )  # status of observatory dome
        self.state.update(
            {
                "Dome_Status_Num": self.config["Dome_Status_Dict"]["Dome_Status"].get(
                    self.Dome_Status, self.default
                )
            }
        )

        self.Home_Status = self.remote_state.get(
            "Home_Status", "NOT_READY"
        )  # status of whether dome needs to be homed
        self.state.update(
            {
                "Home_Status_Num": self.config["Dome_Status_Dict"]["Home_Status"].get(
                    self.Home_Status, self.default
                )
            }
        )
        ...
        self.state.update(
            {
                "Shutter_Status_Num": self.config["Dome_Status_Dict"][
                    "Shutter_Status"
                ].get(self.Shutter_Status, self.default)
            }
        )
        ...
        self.state.update(
            {
                "Control_Status_Num": self.config["Dome_Status_Dict"][
                    "Control_Status"
                ].get(self.Control_Status, self.default)
            }
        )
        ...
        self.state.update(
            {
                "Close_Status_Num": self.config["Dome_Status_Dict"]["Close_Status"].get(
                    self.Close_Status, self.default
                )
            }
        )
        ...
        self.state.update(
            {
                "Weather_Status_Num": self.config["Dome_Status_Dict"][
                    "Weather_Status"
                ].get(self.Weather_Status, self.default)
            }
        )
        ...
        self.state.update(
            {
                "Sunlight_Status_Num": self.config["Dome_Status_Dict"][
                    "Sunlight_Status"
                ].get(self.Sunlight_Status, self.default)
            }
        )
        ...
        self.state.update(
            {
                "Wetness_Status_Num": self.config["Dome_Status_Dict"][
                    "Wetness_Status"
                ].get(self.Wetness_Status, self.default)
            }
        )

        self.state.update(
            {
                "Outside_Dewpoint_Threshold": self.remote_state.get(
                    "Outside_Dewpoint_Threshold", self.default
                )
            }
        )
        self.state.update(
            {
                "Average_Wind_Speed_Threshold": self.remote_state.get(
                    "Average_Wind_Speed_Threshold", self.default
                )
            }
        )
        self.state.update(
            {"Outside_Temp": self.remote_state.get("Outside_Temp", self.default)}
        )
        self.state.update(
            {"Outside_RH": self.remote_state.get("Outside_RH", self.default)}
        )
        self.state.update(
            {
                "Outside_Dewpoint": self.remote_state.get(
                    "Outside_Dewpoint", self.default
                )
            }
        )
        self.state.update({"Pressure": self.remote_state.get("Pressure", self.default)})
        self.state.update(
            {"Wind_Direction": self.remote_state.get("Wind_Direction", self.default)}
        )
        self.state.update(
            {
                "Average_Wind_Speed": self.remote_state.get(
                    "Average_Wind_Speed", self.default
                )
            }
        )
        self.state.update(
            {
                "Weather_Hold_time": self.remote_state.get(
                    "Weather_Hold_time", self.default
                )
            }
        )
        ...
        for fault_code in self.config["Dome_Status_Dict"]["Faults"]:
            if self.Faults & fault_code:
                ...
                self.state.update(
                    {self.config["Dome_Status_Dict"]["Faults"][fault_code]["field"]: 1}
                )
            else:
                # assign the variable to false
                self.state.update(
                    {self.config["Dome_Status_Dict"]["Faults"][fault_code]["field"]: 0}
                )

        self.dome_ok = (
            (
                self.Dome_Status != "FAULT"
            )  # NPL 4-22-24 changed to avoid FAULT not UNKNOWN since we had false alarms
            and (self.Home_Status == "READY")
            and (
                self.Shutter_Status != "FAULT"
            )  # NPL 4-22-24 changed this too from UNKNOWN --> FAULT
...
if __name__ == "__main__":
    ...
    dome = local_dome(wsp_path, config, ns_host="localhost")
