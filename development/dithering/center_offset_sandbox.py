...
for target_name, values in targets.items():
    ...
    print(
        f"target RA/Dec (h,deg)    = ({j2000_coords.ra.hour:.2f}, {j2000_coords.dec.deg:.2f})"
    )
    ...
    pa = (
        target_field_angle
        - config["telescope"]["rotator"]["winter"]["rotator_field_angle_zeropoint"]
        + 90
    )
    ...
    addr_0 = config["observing_parameters"][camname]["base_position"][orientation][
        "addr"
    ]
    xd_0 = config["observing_parameters"][camname]["base_position"][orientation][
        "x_pixel"
    ]
    yd_0 = config["observing_parameters"][camname]["base_position"][orientation][
        "y_pixel"
    ]
    if "s" in addr_0:
        xc_0 = wx - xd_0
