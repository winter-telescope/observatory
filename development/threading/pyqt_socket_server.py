...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 19:20:11 2020

This is an attempt to tie together PyQt and the socket code

Goal:
    
    Run some continuous background processes on timed loops
    Set up a bunch of worker threads in a threadpool that listen for commands
    over TCP/IP and execute anything they receive
...
class timedLoop_1Hz(QtCore.QThread):
    ...
    def counter(self):
        self.index +=1
        """
        # print the index only on evens
        if (self.index % 2) == 0:
            #Then it's even
            print("1 Hz Loop: %d" % self.index)
        """
        print(f"1 Hz Loop: {self.index}")
