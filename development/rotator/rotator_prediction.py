...
ra, dec = "01:37:34.77", "+49:52:48.39"
...
target_field_angle = config["telescope"]["rotator"][camname][
...
parallactic_angle = (
    np.arctan2(
        np.sin(hour_angle), np.tan(lat) * np.cos(dec) - np.sin(dec) * np.cos(hour_angle)
    )
    * 180
    / np.pi
)

predicted_rotator_mechangle = (
    config["telescope"]["rotator"][camname]["rotator_field_angle_zeropoint"]
    - parallactic_angle
    + target_alt
)
